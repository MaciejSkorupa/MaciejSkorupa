...
if ( ! class_exists( 'Awl_Portfolio_Filter_Gallery' ) ) {
	...
	class Awl_Portfolio_Filter_Gallery {		
		...
		// Portfolio Gallery cpt shortcode column before date columns
		public function set_awl_filter_gallery_shortcode_column_name($defaults) {
			$new = array();
			$shortcode = $columns['awl_filter_gallery_shortcode'];  // save the tags column
			unset($defaults['tags']);   // remove it from the columns list

			foreach($defaults as $key=>$value) {
				if($key=='date') {  // when we find the date column
				   $new['awl_filter_gallery_shortcode'] = __( 'Shortcode', PFG_TXTDM );  // put the tags column before it
				}    
				$new[$key] = $value;
			}
			return $new;  
		...
		// Portfolio Gallery cpt shortcode column data
		public function custom_awl_filter_gallery_shodrcode_data( $column, $post_id ) {
			switch ( $column ) {
				case 'awl_filter_gallery_shortcode' :
					echo "<input type='text' class='button button-primary' id='awl-filter-gallery-shortcode-$post_id' value='[PFG id=$post_id]' style='font-weight:bold; background-color:#32373C; color:#FFFFFF; text-align:center;' />";
					echo "<input type='button' class='button button-primary' onclick='return PORTFOLIOCopyShortcode$post_id();' readonly value='Copy' style='margin-left:4px;' />";
					...
					echo "<script>
						function PORTFOLIOCopyShortcode$post_id() {
							var copyText = document.getElementById('awl-filter-gallery-shortcode-$post_id');
							copyText.select();
				...
				<ul id="remove-images" class="sbox">
					...
					if(isset($allimagesetting['image-ids'])) {
						...
						<li class="image">
							...
							?>
							<select class="pfg-filters form-control" name="filters[<?php echo $id; ?>][]" multiple="multiple" id="filters" style="width: 100%;">
								<?php
		...
		public function _pfg_ajax_callback_function($id) {
			...
			<li class="image">
				...
				<select class="pfg-filters form-control" name="filters[<?php echo $id; ?>][]" multiple id="filters" style="width: 100%;">
					<?php
					foreach ($all_category as $key => $value) {
						if($key != 0) {
